...
class DataValidation:
    ...
    def validate_number_of_columns(self,dataframe: pd.DataFrame)-> bool:
        try:
            ...
            logging.info(f"Required number of coloums:{number_of_coloumns}")
            logging.info(f"Data Frame has coloumns: {len(dataframe.columns)}")
            if len(dataframe.columns) == number_of_coloumns:
                return True
            return False
        except Exception as e:
            raise NetworkSecurityException(e,sys)
    ...
    def validate_numerical_columns_exist(self, dataframe: pd.DataFrame)-> bool:
        try:
            ...
            logging.info(f"Data Frame has columns: {list(dataframe.columns)}")

            for col in required_numerical_columns:
                if col not in required_numerical_columns:
                    return False
            return True
        except Exception as e:
            raise NetworkSecurityException(e,sys)
    ...
    def initiate_data_validation(self) -> DataIngestionArtifact:
        try:
            ...
            if not status:
                error_message=f" Train dataframe does not containt all the columns\n"
            
            status = self.validate_number_of_columns(dataframe=test_dataframe)
            if not status:
                error_message=f" Test dataframe does not containt all the columns\n"

            status = self.validate_numerical_columns_exist(dataframe=train_dataframe)
            if not status:
                error_message=f" Train dataframe does not containt all the numerical columns\n"
